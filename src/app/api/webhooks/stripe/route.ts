...
import { STAGE_REPLY_LETTER_ID } from "@/lib/stage-letter";
import { defaultStageReplyDeps, fulfilStageReplyLetter } from "@/lib/stage-reply-fulfilment";

// pdfkit (via @/lib/letter-pdf) reads .afm font files from disk and the Edge
// runtime cannot load these. outputFileTracingIncludes in next.config.ts
// ships them with the bundle for this route too (the config also covers
// /api/webhooks/stripe).
...
// ============== PDF generation ==============
// generatePDF and escapeHtml moved verbatim to src/lib/letter-pdf.ts on
// 2026-09-11 so the Escalation Reply Letter renders the identical PDF.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Escalation Reply Letter (£9.99): same Claude, PDF and Resend pipeline
    // as the appeal letters, with a stage-specific prompt. Handled before the
    // generic path so the first-stage branch below is untouched. The Premium
    // add-on, when bundled, reuses this route's own generateLetter.
    if (session.metadata?.productId === STAGE_REPLY_LETTER_ID) {
      const outcome = await fulfilStageReplyLetter(session, {
        ...defaultStageReplyDeps(),
        generatePremium: (appeal) => generateLetter(appeal),
      });
      if (!outcome.ok) {
        console.warn(`Webhook: stage reply session ${session.id} skipped: ${outcome.reason}`);
      ...
      return NextResponse.json({ ok: true, sent_to: outcome.sentTo });
    }
